...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	// TimeTable Columns names
	private static final String KEY_ID = "id";
	private static final String KEY_NAME_ID = "subjectid";
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_TIMETABLE = "CREATE TABLE IF NOT EXISTS "
				...
				+ KEY_TEACHER_NAME + " TEXT," + KEY_DAY_ID + " INTEGER,"
				+ KEY_LESSON_ID + " INTEGER," + KEY_COLOR_ID + " INTEGER" + ")";
	...
	void addSubject(Subject sub, int Day_ID, int Lesson_ID) {
	...
	// LESEN
	// Einzelnes Subject mit Day_ID und lesson_ID lesen
	public Subject readSubject(int Day_ID, int Lesson_ID) {
		Subject result = null;
		// SQL ABFRAGE
		String selectQuery = "SELECT  * FROM " + TABLE_TIMETABLE + " WHERE "
				+ KEY_DAY_ID + " = " + Day_ID + " AND " + KEY_LESSON_ID + " = "
				+ Lesson_ID;
		...
		if (cursor.moveToFirst()) {
			...
			result = new Subject(NAME_ID, NAME, TEACHER, COLOR_ID);
		}
	...
	// Alle Subjects in Liste speichern
	...
	public LinkedList<int[]> readAllDaysAndLessons() {
	...
	public int updateSubject(Subject sub, int Day_ID, int Lesson_ID) {
		...
		ContentValues values = new ContentValues();
		values.put(KEY_ID, sub.getName());
		values.put(KEY_NAME_ID, sub.getID());
		values.put(KEY_NAME, sub.getName());
		// Teacher fehlt
		values.put(KEY_DAY_ID, Lesson_ID);
		values.put(KEY_LESSON_ID, Day_ID);
	...
	public void deleteRow(int EntryID) {
	    SQLiteDatabase db = this.getWritableDatabase();
	    db.delete(TABLE_TIMETABLE, KEY_ID +" = " + String.valueOf(EntryID), null);
	    db.close();
	}
